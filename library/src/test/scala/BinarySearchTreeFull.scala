...
//  }
  ...
  val ioExamples = List(
    ((Leaf(), Node(Leaf(), 5, Leaf())) -> Node(Leaf(), 5, Leaf())),
    (
      (Node(Leaf(), 4, Leaf()), Node(Leaf(), 5, Leaf())) ->
        Node(Node(Leaf(), 4, Leaf()), 5, Leaf())
    ),
    (Node(Node(Leaf(), 3, Leaf()), 4, Leaf()), Node(Leaf(), 5, Leaf())) ->
      Node(Node(Leaf(),3,Node(Leaf(),4,Leaf())),5,Leaf()),
    (Node(Leaf(), 4, Node(Leaf(), 5, Leaf())), Node(Leaf(), 8, Leaf())) ->
      Node(Node(Node(Leaf(),4,Leaf()),5,Leaf()),8,Leaf()),
    (Node(Node(Leaf(), 3, Leaf()), 4, Node(Leaf(), 5, Leaf())), Node(Leaf(), 8, Leaf())) ->
      Node(Node(Node(Leaf(),3,Node(Leaf(),4,Leaf())),5,Leaf()),8,Leaf())
  )
  
  def union(t1: Tree, t2: Tree): Tree = t1 match {
    case Leaf() => t2
    case Node(l, k, r) =>
      insert( union(l, union(r, t2)), k)
  }
  
  def main(args: Array[String]): Unit = {

    for ( ((t1, t2), o) <- ioExamples ) {
      assert ( union(t1, t2) == o, "Got: " + union(t1, t2) ) 
    }
