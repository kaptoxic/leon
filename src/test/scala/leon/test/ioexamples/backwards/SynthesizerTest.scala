...
class SynthesizerTest extends FunSuite {
  ...
  val inputVar = funDef.params.head.toVariable
  val codeGenEval = new CodeGenEvaluator(sctx, sctx.program)
  
  val consClass = program.caseClassDef("Cons").typed
  val pairClass = program.caseClassDef("Pair").typed
  ...
  val mergeFun = program.definedFunctions.find(_.id.name == "merge").get.typed
  val sortFun = program.definedFunctions.find(_.id.name == "sort").get.typed
  val splitFun = program.definedFunctions.find(_.id.name == "split").get.typed
  ...
  val l21tol2 = listTail
  val l213tol21 =
    CaseClassSelector(pairClass, FunctionInvocation(splitFun, List(inputVar)),
      pairClass.fields.find(_.id.name == "fst").get.id)
  val l213tol3 = listTail
  
  val termSynthesizer = new TermSynthesizer(sctx, problem)
  val synthesizer = new Synthesizer(codeGenEval, funDef, termSynthesizer)
  ...
  test("synthesizer.expand or-node") {
    ...
    result match {
      case (el: OrFragment) :: Nil =>
        ...
        el.fragment should be (IntLiteral(1))
      case _ =>
        fail
    }
  }
...
//        root addChild consNode
  ...
  test("synthesizer.propagate mergesort example, unsuccessfull propagation") {
    ...
    exampleRoot.isSolved should be (false)
  }
...
//      mergeNode.setSolved(m1Node)
  ...
  test("synthesizer.explore mergesort example, queue") {
    ...
    def inverser(name: String)(frag: Expr) = (name, frag) match {
      case ("merge", `l21`) => List( List(l2, l1) )
      case ("merge", `l12`) => List( List(l1, l2) )
      case ("merge", `l213`) => List( List(l21, l3) )
      case ("merge", `l123`) => List( List(l12, l3) )
      case _ =>
        fail("cannot inverse " + name + " for " + frag)
    }
    ...
    val root = new RootStep()
    val example1 = (l21, l12)
    val exampleRoot1 = new RootFragment(root, example1)
    
    val stepTreeQueue =
      synthesizer.explore(exampleRoot1, functions, subexpressions, inverser)
      
    stepTreeQueue.size should be (2)
    root.isSolved should be (true)
    exampleRoot1.isSolved should be (true)
    
    def inverser2(name: String)(frag: Expr) = (name, frag) match {
      case ("rec", `l12`) => List( List( l21 ) )
      case _ =>
        inverser(name)(frag)
    }
    
    def functions2(e: Expr) = e match {
      case `l12` => List("rec") ++ functions(e)
      case _ => functions(e)
    }
    
    val example2 = (l213, l123)
    val exampleRoot2 = new RootFragment(root, example2)
    val mapFromStepToFragment =
      synthesizer.propagate(exampleRoot2, subexpressions, inverser2)
      
    root.isSolved should be (false)
    mapFromStepToFragment.size should be (2)
    
    val fragmentQueue = stepTreeQueue map mapFromStepToFragment
    val stepTreeQueue2 =
      synthesizer.explore(exampleRoot2, functions2, subexpressions, inverser2, fragmentQueue)
      
    withClue("Step tree: " + FormatStepGraph(root, true) + "\nQueue: " + stepTreeQueue2.mkString(", ")) {
    	stepTreeQueue2.size should be (4)
    }
    exampleRoot2.isSolved should be (true)
    root.isSolved should be (true)

    synthesizer.propagate(exampleRoot1, subexpressions, inverser2)
    exampleRoot1.isSolved should be (true)
    root.isSolved should be (true)
    ...
    def cst(op: String, exprs: List[Expr]): Expr = (op, exprs) match {
      ...
      case ("root", List(headExpr)) =>
        headExpr
      case ("none", List(headExpr)) =>
        headExpr
      case ("rec", _) =>
        FunctionInvocation(sortFun, exprs)
      case _ => fail("did not process: " + (op, exprs))
    }
     
    withClue("Step tree: " + FormatStepGraph(root, true) + "\nQueue: " + stepTreeQueue2.mkString(", ")) {
      val solution = root.getSolution(cst)
      solution should be (
        FunctionInvocation(mergeFun, List(
      		FunctionInvocation(sortFun, List(
    		    l213tol21
  		    )),
          listTail
    		))
...
//      
//        synthesizer.synthesize(examples, functions, subexpressions, cst, inverser)
//  	      be (Some(inputVar))	    
//      }
//      
//      val termSynthesizer = new TermSynthesizer(sctx, problem)
...
////          case ("merge", `l21`) => List( List(l2, l1) )
//          case ("merge", `l12`) => List( List(l1, l2) )
//          // needed due to case in which we already saw it
//          case ("rec", _) => List()
//          case ("merge", _) => List()
////          case _ =>
////            fail
...
//        }
//  
//        synthesizer.synthesize(examples, functions, subexpressions, cst, inverser) should
