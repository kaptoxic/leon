 ...
 */
class FlattenTreeTest extends FunSuite with Matchers with Inside with HasLogger {
  ...
  val problems = forFile(ioExamplesTestcaseDir + "FlattenTree.scala").toList
  ...
  val consClass = program.caseClassDef("IntNode").typed
  ...
  test("test synthesizer") {
    ...
    body.toString shouldBe
      """|if ((l == Nil)) {
         |  IntNil
         |} else {
         |  IntNode(IntNode(IntNil, l.value.fst, IntNil), l.value.snd, rec(l.right))
         |}""".stripMargin
