 ...
 */
class Flatten4Test extends FunSuite with Matchers with Inside with HasLogger {
  ...
  val problems = forFile(ioExamplesTestcaseDir + "Flatten10.scala").toList
...
//         |  IntNil
//         |} else if ((l == Nil)) {
//         |  IntCons(l.head.fst, IntCons(l.head.snd, IntCons(l.head.thd, IntCons(l.head.fth, ?(l.tail)))))
//         |} else {
//         |  ()
//         |}""".stripMargin
         
    ExprOps.formulaSize(body) shouldBe 27
