...
class DesugarTest extends FunSuite with Matchers with Inside with HasLogger {
  ...
  def getSecondASTs(size: Int) = {
    val ms = new scope.AccumulatingScope
    val enum = constructEnumerator(secondsASTops, fromOpToOperandParam _,
      desugarModule, secondLiterals)(ms)
  ...
  ignore("datastructure generation") {
    ...
    getFirstASTs(5) should have size 100
  ...
  ignore("datastructure generation, static typed") {
    ...
    getSecondASTs(7).map(_.toString) should
      contain("Ite(CheckType(Literal(2), Literal(5)), Plus(Literal(2), Literal(5)))")
