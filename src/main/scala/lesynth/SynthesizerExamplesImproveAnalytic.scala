...
import leon.synthesis.ioexamples.Differencer

case class SynthesizerExamplesImproveAnalytic(
    ...
    breakable {
      while (keepGoing) {
        ...
        
        {
          ...
          assert(counterExamples.size > 0)
          val inputSubexpressions =
            for ( InputOutputExample(inputs, output) <- counterExamples ) yield {
              assert(inputs.values.size == 1)
              val inputDecompose = mapOfSubexpressions(inputs.values.head)
              val outputExplained = constructFragment(output, inputDecompose)
              outputExplained
            }
          
          println("Found subexpressions")
          println(inputSubexpressions.mkString("\n"))
          assert(inputSubexpressions.distinct == inputSubexpressions)
          ...
          val diffs = Differencer.differences(inputs.toList, holeFunDef.args.head.toVariable)
          println("diffs:\n" + diffs.mkString("\n"))
