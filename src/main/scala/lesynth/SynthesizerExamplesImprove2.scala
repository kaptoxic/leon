...
case class SynthesizerExamplesImprove2(
  ...
  def synthesize: Report = {
    ...
    var (it1, it2) = snippetsIterator.duplicate
    ...
    breakable {
      while (keepGoing) {
        ...
        if (iteration > StartAlgorithmicAfter) {
          ...
          val leafClass = program.caseClassDef("Leaf")
          val leaf = CaseClass(leafClass, Nil)
          val nodeClass = program.caseClassDef("Node")
          val pairClass = program.caseClassDef("Pair")
          val t1Field = pairClass.fieldsIds.find({ x => x.name == "t1" }).get

          allDeclarations = //variableRefiner.updateDeclarations(id, loader.classMap(classDef.id), allDeclarations)
          (for (dec <- allDeclarations)
            yield dec match {
              case LeonDeclaration(inSynthType, _, decClassType,
                ImmediateExpression(_, t1AccessExpr @ CaseClassSelector(`pairClass`, _, `t1Field`))
              )
//              if t1Field.name == "t1"
              =>                
                println(t1AccessExpr)
                ((
                  for (field <- nodeClass.fields)
                    yield LeonDeclaration(
                      ImmediateExpression( "Field(Node." + field.id + ")",
                        CaseClassSelector(nodeClass, t1AccessExpr, field.id) ), 
        ...
        println("All declarations: " + allDeclarations.mkString("\n"))
          ...
          ;
        // iterator for algorithmic part
        val dup = snippetsIterator.duplicate
        it1 = dup._1
        it2 = dup._2
        snippetsIterator = it1
        snippetsIteratorCopy = it2
        ...
        interactivePause
          ...
          assert(restOfExamples.size == 3, restOfExamples.size)
          ...
          implicit def intToLiteral(v: Int) = IntLiteral(v)

          leftPart =
            CaseClass(nodeClass, List(
              leaf, 5, leaf
            ))
          rightPart =
            IntLiteral(4)
...
//          
          ...
          val candidates = snippetsIteratorCopy.take(300).toList
          var passCandidates =
...
//                (candidate.getSnippet -> res.result.get)
              if ( res.result.get == ex.asInstanceOf[InputOutputExample].output)
                Some(candidate)
              else
                None
            }
          ...
          reporter.info("resultsOnOtherInputs: " + resultsOnOtherInputs.mkString("\n"))
            resultsOnOtherInputs.flatten.toSet
          }
            
          val commonResult = sets.reduce( (x, y) => x.intersect(y) )

          reporter.info("commonResult: " + commonResult)
          interactivePause
//          
