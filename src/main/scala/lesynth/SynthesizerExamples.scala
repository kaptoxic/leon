        ...
        breakable {
          ...
          for (
            ...
            if (!refiner.isAvoidable(snippetTree, problem.as)) {
              ...
              val (passed, failed) = countPassedExamples(snippetTree)
              val passCount = passed.size
              ...
              if (passCount == exampleRunner.counterExamples.size) {
                ...
                found = true
                break
//                if (tryToSynthesizeBranch((snippetTree, passed, failed))) {
...
//                }
                ...
                if (passCount > 0) {
                  ...
                  pq.enqueue(((snippetTree, passed, failed), 100 + (passCount * iteration) - snippet.getWeight.toInt))
                } else {
            ...
            if (numberOfTested >= numberOfTestsInIteration * noBranchFoundIteration) {
              ...
              fine("Queue contents: " + pq.toList.take(10).map(_._1._1).mkString("\n"))
              fine({ if (pq.isEmpty) "queue is empty" else "head of queue is: " + pq.head._1._1 })
              //interactivePause

              // go and check the topmost numberOfCheckInIteration
              for (i <- 1 to math.min(numberOfCheckInIteration, pq.size)) {
                val queueValue @ (nextSnippet, passed, failed) = pq.dequeue._1
                fine("dequeued nextSnippet: " + nextSnippet)
                //                fine("for: " + snippetTree + "passed: " + passed.mkString(",") + "failed: " + failed.mkString(","))
                fine("for: " + nextSnippet)
                //interactivePause

                if (tryToSynthesizeBranch(queueValue)) {
                  noBranchFoundIteration = 0
                  break
                }
