...
//  var recursiveNodeSet: MutableSet[env.Node] = _
  ...
  def apply(root: SimpleNode) = {
    ...
    val transformed = transform(root, emptyContext, goalType, Set.empty, Map(goalType -> List(root)))
  ...
  private def transform(node: SimpleNode, oldContext: Context,
    goalType: DomainType, visited: Set[env.Node], visitedMap: Map[DomainType, List[env.Node]]): Streamable[lambda.Node] = {
    // logging
                    ...
                    nodeToCheck match {
                      ...
                      case nprime: SimpleNode /*if nprime.getType == parameterTypeInSynth*/ =>
                        ...
                        transform(nprime, context, parameterType, 
                            visited + nodeToCheck, visitedMap + (parameterType -> (nodeToCheck :: visitedMap.getOrElse(goalType, Nil)))) :: list
...
//	                  	(paramInitStream, Set(recursiveParamLast)))
    ...
    def getMatchingTypeFromDeclaration: List[Streamable[Node]] = {
      ...
      (List[Streamable[Node]]() /: node.getDecls.filter(
        d => !(d.getSimpleName contains "addToPers")
        )) {
        (list, declaration) =>
          {
            declaration match {
